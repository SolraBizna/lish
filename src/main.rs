...
struct Shell {
    ...
    interactive: bool,
    batch_failed: Option<NonZeroI32>,
}
...
impl Shell {
    ...
    /// program should exit.
    fn input(&mut self, mut input: String) -> Option<i32> {
        ...
        let mut forced_job_name = None;
        // Try to parse a custom job name.
        let mut it = input.char_indices();
        let mut found_colon = true;
        for (_, c) in &mut it {
            if c.is_ascii_whitespace() { break }
            else if c == ':' { found_colon = true; }
            else { found_colon = false; }
        }
        if found_colon {
            let mut cutoff = None;
            for (i, c) in it {
            ...
            if let Some(cutoff) = cutoff {
                let new_input = input[cutoff..].to_owned();
                input.truncate(cutoff);
                while input.ends_with(|x: char| x.is_ascii_whitespace()) {
                    input.pop();
                }
                if input.ends_with(":") {
                    input.pop();
                }
                forced_job_name = Some(input);
                input = new_input;
            }
        }
        // Parse what's left, either for direct execution or for extracting a
        // job name.
        ...
        match our_parse {
            Ok(mut elements) => {
                ...
                if cfg!(not(unix)) || env::var("NOSH").map(|x| !x.is_empty() && x != "0").unwrap_or(false) {
                    Job::spawn_nosh(self, forced_job_name.unwrap_or_else(|| job_name.clone()), job_name, input, elements);
                    self.update_status();
                    return None;
                }
            },
        ...
        Job::spawn_sh(self, forced_job_name.unwrap_or(job_name), input);
        self.update_status();
        return None;
    }
        ...
        match how {
            Ok(x) => {
                if x.success() {
                    ...
                    if self.batch_failed.is_none() {
                        self.batch_failed = NonZeroI32::new(x.code().unwrap_or(255));
                    }
                }
            },
            Err(x) => {
    ...
    fn next_target(&mut self) {
        ...
        loop {
            ...
            match self.target_job {
                None => {
                    if self.interactive || self.jobs.is_empty() {
                        break
                    }
                },
    ...
    fn update_prompt(&mut self) {
        match self.target_job {
            None => {
                if !self.interactive {
                    if !self.any_jobs_alive() {
                        // do nothing, we're about to exit
                    }
                    else {
                        self.next_target();
                        self.update_prompt() // TODO: maybe infinite loop?
                    }
                }
                else {
                    self.output.prompt(liso!(fg=green, bold, &self.cwd_name, "> ", reset), true, false)
                }
            },
            Some(_) if !self.interactive && self.count_jobs_alive() <= 1 => {
                self.output.prompt(liso!(fg=blue, bold, ">", reset), true, false)
            },
    ...
    fn quit(&mut self) -> Option<i32> {
        ...
        self.builtin_exit(vec!["0".to_string()])
    }
    ...
    fn builtin_exit(&mut self, args: Vec<String>) -> Option<i32> {
        let status = match args.len() {
            0 => {
                if self.interactive && self.any_jobs_alive() {
                    self.interactive = false;
                    self.ok("Entering batch mode. Will exit when all jobs have completed.");
                    self.next_target();
                    return None
                }
                Some(0)
        ...
        if !self.jobs.is_empty() {
            ...
            if status == 0 {
                self.err("Use `exit` with no parameters to enter Batch Mode.");
            }
            return None
        }
        return Some(status)
    ...
    fn print_usage(&self) {
        self.ok("Usage: lish [options] [jobs]");
        self.ok("Known options are:");
        self.ok("-i: Enter interactive mode, even if jobs are specified");
        self.ok("-h: This help");
        self.ok("All other arguments are command lines to enter.");
        self.ok("Each argument is a separate command line, so add a layer of quotation marks to be safe.");
    }
    fn set_interactive(&mut self, interactive: bool) {
        self.interactive = interactive;
    }
    fn batch_over(&self) -> bool {
        !self.any_jobs_alive() && !self.interactive
    }
    fn any_jobs_alive(&self) -> bool {
        self.jobs.iter().any(Option::is_some)
    }
    fn count_jobs_alive(&self) -> usize {
        self.jobs.iter().filter_map(|x| if x.is_some() { Some(()) } else { None }).count()
    }
}
        ...
        if arg.starts_with("-") {
            if arg == "-i" {
                if interactive == Some(true) {
                    shell.warn("-i specified more than once.");
                }
                ...
                shell.err(&format!("Unknown command line option: {:?}", arg));
                shell.print_usage();
                return 1;
            }
        }
